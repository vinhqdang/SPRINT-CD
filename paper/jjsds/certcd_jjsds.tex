...
The real-data experiments deliver the paper's own counterexamples. Where the
certifying null rests on the Markov condition alone, the method behaves as
advertised on real data. Where it rests on a parametric functional model, it
does not: on Sachs it is confidently and systematically wrong, with nothing
in its output to say so; on the Tübingen pairs it mostly declines to commit,
correctly, but is no better than chance on the minority of pairs where it
does. A certificate is only ever as good as the null it certifies against,
