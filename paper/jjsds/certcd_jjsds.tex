...
instances that violate $\delta$-strong faithfulness, CERT-CD stays within its
budget while methods that delete on non-rejection lose a true edge in most
runs; on premise-satisfying instances it orients $96\%$ of certified edges,
against a Markov-equivalence-class ceiling of $33\%$. On the observational
...
graph would like to know which of its features the data support. Methods in
the constraint-based tradition---PC, FCI and their descendants
\citep{spirtes2000causation,colombo2012learning}---answer this question
...
\item An empirical study (Section~\ref{sec:experiments}) that stratifies
  ...
  observational protein-signalling data of \citet{sachs2005causal}, where the
  adjacency and orientation halves of the output behave very differently.
\end{enumerate}
...
optional stopping; the converse fails, and the distinction matters for
...
\citep{lindon2024anytime}, stratified counts \citep{turner2023safe}---and has
...
by d-separation in $\Gstar$. \emph{Adjacency-faithfulness}
...
inherit anything, and would need its own analysis.
\end{remark}
...
\citet{wang2022false} applies to e-values under arbitrary dependence and would
...
certified skeleton, apply the FCI orientation rules R1--R4 \citep[in the
formulation of][]{zhang2008completeness} to the certified edges only, and
...
control the FDR of orientations; \citet{shimizu2008use} test direction of
...
v-structure is unorientable by it in principle. \citet{shimizu2008use} escapes
...
method can orient an edge lying in no v-structure. ``Time-uniform'' means the
...
realised-to-nominal ratio anywhere in the table is $0.95$.}%
\label{tab:primitive}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
observations (discarded), prior scale $g = 0.5$ after standardisation,
